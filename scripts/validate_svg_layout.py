...
def classify_text_fit_issue(diagnosis, font_size):
    ...
    if horizontal > major_horizontal or vertical > major_vertical:
        return (
            "info",
            "TEXT_OVERFLOW_MAJOR",
            "Text likely overflows its container — verify visually via PNG; NOT a blocking issue",
            "major",
        )
...
def validate_file(path, margin=MARGIN, quick_mode=False):
    ...
    # Local image references must resolve from the produced SVG, not from the source canvas.
    # This catches canvases copied to another directory without rewriting relative hrefs.
    image_nodes_for_paths = list(root.findall(".//{http://www.w3.org/2000/svg}image")) + list(root.findall(".//image"))
    for idx, image_node in enumerate(image_nodes_for_paths, 1):
        href = (image_node.get("href")
                or image_node.get("{http://www.w3.org/1999/xlink}href")
                or "").strip()
    ...
    # Extract metadata early for both quick_mode and full validation
    meta = extract_metadata_comment(content)

    # ── quick_mode: skip text-fit estimation, margin, overlap, density, palette ──
    if quick_mode:
        all_issues = errors + warnings + infos
        e_count = len(errors)
        w_count = len(warnings)
        i_count = len(infos)
        status = "fail" if e_count > 0 else ("warning" if w_count > 0 else "pass")
        return {
            "file": str(path),
            "status": status,
    ...
    # ── Palette checks removed — color decisions verified by visual review ──
    ...
    for idx, node in enumerate(text_nodes, 1):
        ...
        if outside_safe_margin(tbox, margin):
            ...
            W(
                "UNSAFE_MARGIN",
                f"Text outside {margin:.0f}px safe margin — verify visually via PNG",
                target=f"text_{idx}",
                detail=box_to_dict(tbox),
                diagnosis=diagnosis,
                recommended_fix=recommended_fix,
...
def main():
    parser = argparse.ArgumentParser(
        description="Heuristic SVG layout validator — compatibility, text, palette, metadata, images, safe areas, rhythm."
    )
    ...
    parser.add_argument("--manifest", default="", help="Path to page_manifest.json for cross-validation")
    parser.add_argument("--batch", default="", help="batch_id from page_manifest.json; requires --manifest")
    parser.add_argument("--layout-plan", default="", help="Optional approved layout_plan.json for template-layout binding")
    parser.add_argument("--fidelity-template", default="", help="Optional reviewed fidelity template_registry.json")
    parser.add_argument("--self-check", action="store_true", help="Quick self-check mode: only P0 hard errors, skip text-fit/margin/overlap/density")
    parser.add_argument("--file", default="", help="Single SVG file to validate (alternative to svg_dir)")
    args = parser.parse_args()

    if args.batch and not args.manifest:
        print(json.dumps({"status": "fail", "summary": {"errors": 1, "warnings": 0, "infos": 0},
                          "reports": [{"file": "", "status": "fail", "issues": [
                              issue("error", "MANIFEST_REQUIRED_FOR_BATCH", "--batch requires --manifest")]}]},
                         ensure_ascii=False, indent=2))
        return 1

    quick_mode = args.self_check

    # Determine SVG source: --file takes priority, then svg_dir
    if args.file:
        svg_path = Path(args.file)
        if not svg_path.is_file() or svg_path.suffix.lower() != ".svg":
            print(json.dumps({"status": "fail", "summary": {"errors": 1, "warnings": 0, "infos": 0},
                              "reports": [{"file": str(svg_path), "status": "fail",
                                           "issues": [issue("error", "INVALID_SVG_FILE", f"Not a valid SVG file: {svg_path}")]}]},
                             ensure_ascii=False, indent=2))
            return 1
        svg_files = [svg_path]
        manifest_data = None
        ...
        if args.manifest:
            try:
                ...
                print(json.dumps({"status": "fail", "summary": {"errors": 1, "warnings": 0, "infos": 0},
                                  "reports": [{"file": str(args.manifest), "status": "fail", "issues": [
                                      issue("error", "INVALID_MANIFEST", str(exc))]}]}, ensure_ascii=False, indent=2))
                return 1
    ...
    # Validate each file
    reports = [validate_file(f, margin=args.margin, quick_mode=quick_mode) for f in svg_files]
    for report, svg_file in zip(reports, svg_files):
        report["input_sha256"] = hashlib.sha256(Path(svg_file).read_bytes()).hexdigest()

    if args.batch and manifest_data:
        expected = set(get_batch_pages(manifest_data, args.batch))
        actual = {Path(path).stem for path in svg_files}
        if expected != actual:
            reports[-1]["issues"].append(issue(
                "error", "BATCH_PAGE_SET_MISMATCH",
                f"Batch {args.batch} expected {sorted(expected)} but validated {sorted(actual)}",
                target=args.batch,
            ))
    ...
    if args.layout_plan:
        try:
            layout_plan = json.loads(Path(args.layout_plan).read_text(encoding="utf-8"))
            layout_by_page = {page.get("page_key"): page for page in layout_plan.get("pages", []) if isinstance(page, dict)}
        except Exception as exc:
            reports[-1]["issues"].append(issue("error", "INVALID_LAYOUT_PLAN", str(exc), target="layout_plan"))
            reports[-1]["summary"]["errors"] += 1
            reports[-1]["status"] = "fail"
    ...
    if layout_by_page:
        for report in reports:
            ...
            planned = layout_by_page.get(page_key, {})
            expected_labels = {
                str(region.get("label", "")).strip()
                for region in planned.get("wireframe", [])
                if isinstance(region, dict)
                and str(region.get("label", "")).strip()
                and str(region.get("zone", "")).strip().lower() != "background"
            }
            try:
                svg_root = ET.parse(report["file"]).getroot()
                actual_labels = {
                    str(node.attrib.get("data-wireframe-label", "")).strip()
                    for node in svg_root.iter()
                    if str(node.attrib.get("data-wireframe-label", "")).strip()
                }
            except Exception:
                actual_labels = set()
            ...
            if missing:
                report["issues"].append(issue(
                    "error", "WIREFRAME_REGION_MISSING",
                    f"Approved wireframe regions are not represented in SVG: {', '.join(missing)}",
                    target=page_key,
                ))
    ...
    if args.fidelity_template:
        try:
            ...
            component_map = {item.get("component_id"): item for item in template.get("components", []) if isinstance(item, dict)}
            ...
            for report in reports:
                ...
                planned_layout = layout_by_page.get(page_key, {}).get("template_layout_id", "")
                if not planned_layout:
                    report["issues"].append(issue("error", "TEMPLATE_LAYOUT_NOT_PLANNED",
                        "Fidelity mode requires layout_plan.template_layout_id", target=page_key))
                    report["summary"]["errors"] += 1
                    report["status"] = "fail"
                elif layout_id != planned_layout:
                    report["issues"].append(issue("error", "TEMPLATE_LAYOUT_MISMATCH",
                        f"SVG layout {layout_id!r} does not match approved template layout {planned_layout!r}", target=page_key))
                    report["summary"]["errors"] += 1
                    report["status"] = "fail"
                if not layout_id:
                    report["issues"].append(issue("error", "FIDELITY_LAYOUT_NOT_DECLARED",
                        "SVG does not declare data-layout-id", target=page_key))
                    report["summary"]["errors"] += 1
                    report["status"] = "fail"
                    continue
                layout = layouts.get(layout_id)
                if layout is None:
                    report["issues"].append(issue("error", "UNKNOWN_FIDELITY_LAYOUT",
                        f"SVG declares an unknown fidelity layout: {layout_id}", target=page_key))
                    report["summary"]["errors"] += 1
                    report["status"] = "fail"
                    continue
                canvas_file = layout.get("canvas_file", "")
                if strict_components and not canvas_file:
                    report["issues"].append(issue("error", "FIDELITY_CANVAS_NOT_BOUND",
                        f"Layout {layout_id} does not declare canvas_file", target=page_key))
                    report["summary"]["errors"] += 1
                    report["status"] = "fail"
                elif canvas_file:
                    canvas_path = fidelity_path.parent / canvas_file
                    if not canvas_path.is_file():
                        report["issues"].append(issue("error", "FIDELITY_CANVAS_MISSING",
                            f"Layout canvas is missing: {canvas_file}", target=page_key))
                        report["summary"]["errors"] += 1
                        report["status"] = "fail"
                    else:
                        ...
                        if not expected_lock or canvas_lock != expected_lock:
                            report["issues"].append(issue("error", "FIDELITY_CANVAS_INTEGRITY_ERROR",
                                f"Layout canvas lock hash is stale for {layout_id}", target=page_key))
                            report["summary"]["errors"] += 1
                            report["status"] = "fail"
                        elif page_lock != expected_lock:
                            report["issues"].append(issue("error", "FIDELITY_LOCKED_LAYER_MISMATCH",
                                f"SVG changed the locked layer of canvas {layout_id}", target=page_key))
                            report["summary"]["errors"] += 1
                            report["status"] = "fail"
                used_nodes = {}
                ...
                for component_id in sorted(used - set(component_map)):
                    report["issues"].append(issue("error", "UNKNOWN_FIDELITY_COMPONENT",
                        f"SVG uses unknown template component {component_id}", target=page_key))
                    report["summary"]["errors"] += 1
                    report["status"] = "fail"
                for component_id in layout.get("required_components", []):
                    if component_id not in used:
                        report["issues"].append(issue("error", "REQUIRED_FIDELITY_COMPONENT_MISSING",
                            f"Layout {layout_id} requires template component {component_id}", target=page_key))
                        report["summary"]["errors"] += 1
                        report["status"] = "fail"
                if strict_components:
                    for component_id, nodes in used_nodes.items():
                        ...
                        if component.get("geometry_policy", "fixed") == "fixed" and len(nodes) != 1:
                            report["issues"].append(issue("error", "FIDELITY_FIXED_COMPONENT_DUPLICATED",
                                f"Fixed template component {component_id} must appear exactly once", target=page_key))
                            report["summary"]["errors"] += 1
                            report["status"] = "fail"
                        for node in nodes:
            ...
            reports[-1]["issues"].append(issue("error", "INVALID_FIDELITY_TEMPLATE", str(exc), target="fidelity_template"))
            reports[-1]["summary"]["errors"] += 1
            reports[-1]["status"] = "fail"
