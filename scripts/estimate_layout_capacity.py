...
def flatten_copy(value):
    """Flatten arbitrary semantic final_on_slide structures without losing text."""
    ...
    if isinstance(value, list):
        return [text for item in value for text in flatten_copy(item)]
    if isinstance(value, dict):
        return [text for item in value.values() for text in flatten_copy(item)]
    return []
...
def kept_segments(layout_page, content_page):
    ...
    if isinstance(final_copy, dict):
        ...
        other_copy = {key: value for key, value in final_copy.items() if key not in {"title", "subtitle"}}
        main_text = "\n".join(flatten_copy(other_copy))
        if main_text.strip():
